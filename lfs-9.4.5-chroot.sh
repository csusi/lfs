#!/bin/bash
echo ""
echo "### (BLFS) libtasn1-4.6 (0.2 SBU - chrooted to lfs partition as 'root')"
echo "### ====================================================================="
...
LFS_SECTION=9.4.5
LFS_SOURCE_FILE_PREFIX=libtasn1
LFS_BUILD_DIRECTORY=    
...
time {
	...
	make -C doc/reference install-data-local	&> $LFS_LOG_FILE-install-localdata.log
}
...
echo "BLFS $LFS_SOURCE_FILE_PREFIX compiled on $(date -u)" >> /build-logs/0-milestones.log
