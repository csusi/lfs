#!/bin/bash
echo ""
echo "### (BLFS) sgml-common-0.6.3  (0.1 SBU - chrooted to lfs partition as 'root')"
echo "### ====================================================================="
...
LFS_SECTION=9.4.12
LFS_SOURCE_FILE_PREFIX=sgml-common
LFS_BUILD_DIRECTORY=    
...
cd /sources
# test_only_one_tarball_exists  # Ships as a .tgz not tarball causes this to fail.
LFS_SOURCE_FILE_NAME=$BLFS_SOURCE_FILE_NAME  
check_MD5_sums
extract_tarball ""
...
time {
	...
	make $LFS_MAKE_FLAGS docdir=/usr/share/doc install 	&> $LFS_LOG_FILE-make-install.log 
	
	echo "*** Performing Post-Make Tasks ... $BLFS_SOURCE_FTP_FILE"
	install-catalog --add /etc/sgml/sgml-ent.cat \
		/usr/share/sgml/sgml-iso-entities-8879.1986/catalog &> $LFS_LOG_FILE-make-installcatalog-1.log 
		
	install-catalog --add /etc/sgml/sgml-docbook.cat \
		/etc/sgml/sgml-ent.cat								&> $LFS_LOG_FILE-make-installcatalog-2.log 
}
