#!/bin/bash
echo ""
echo "### (BLFS) UnZip-6.0  (0.1 SBU - chrooted to lfs partition as 'root')"
echo "### ====================================================================="
...
time {
	...
	make $LFS_MAKE_FLAGS -f unix/Makefile generic         			&> $LFS_LOG_FILE-make.log
	
	echo "*** Running Make Check ... $BLFS_SOURCE_FTP_FILE"
	### None

	echo "*** Running Make Install ... $BLFS_SOURCE_FTP_FILE"
	make $LFS_MAKE_FLAGS prefix=/usr MANDIR=/usr/share/man/man1 \
		-f unix/Makefile install 	&> $LFS_LOG_FILE-make-install.log 
